...
public class GameModeratorImpl implements GameModerator {
    ...
    public int playerScore(Game game, User player) {
        ...
        for(int row = 0; row < rowSize; row++) {
            for(int col = 0; col < colSize; col++) {
                if (playerCellValue == getCellValue(game.getRows(), row, col)) score++;
            }
        }
        return score;
    }
    ...
    private MoveScore moveScoreForNewPiece(List<CellIterator> iterators, CellLocation newCellLocation, CellValue newCellValue){
    ...
    public boolean isBoardFull(List<MatrixRow> rows) {
        ...
        for(int row = 0; row < rowSize; row++) {
            for(int col = 0; col < colSize; col++) {
                if (getCellValue(rows, row, col) == CellValue.EMPTY) return false;
            }
        }
        return true;
    }
