...
public class G2RestAwayGame extends AwayGame<UUID> {
	...
	protected CellLocation makeAMove(String hostName, int port, UUID identifier, CellLocation cellLocation) {
		...
		G2CellLocation g2CellLocation = restTemplate.exchange(String.format("%s:%s/game/postmove?x=%s&y=%s", hostName,
				port, cellLocation.getRow(), cellLocation.getCol()), HttpMethod.GET, null, G2CellLocation.class)
				.getBody();
